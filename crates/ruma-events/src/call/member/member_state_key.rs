...
impl CallMemberStateKey {
    ...
    ///
    /// The MemberId is a combination of the DeviceId, the UserId and the session
    /// (session.application and session.id) and the user id. Dependent on the parameters the
    /// correct CallMemberStateKey will be constructed.
    ...
    ///
    /// It is recommended to not use the state key to get the user id, but rather use sender field.
